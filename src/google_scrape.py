...
def run_search(
    ...
    while not end_of_results:
        driver.get(encoded_query)
        sleep(3)
        current_html: str = driver.find_element(By.XPATH, "//body").get_attribute('outerHTML')
        while "captcha" in current_html:
            logging.info("Captcha detected in current HTML, waiting 10 seconds and retrying.")
            sleep(10)
            driver.get(encoded_query)
            current_html: str = driver.find_element(By.XPATH, "//body").get_attribute('outerHTML')
    ...
    logging.info(f"End of results reached for {search_params["q"]}, found {len(search_results)} results.")
